...
Assume than the array is sorted in non-decreasing order. Write efficient functions to 
...
{ 
if(mid + 1 <= high && x <= arr[mid+1]) //make sure we want arr[mid+1]
return mid + 1; 
else 
...
if(mid - 1 >= low && x > arr[mid-1]) //make sure we want arr[mid-1]
return mid; 
else 
