...
in this directory, you need to find out all the groups of duplicate files in the file system in terms of their paths.
...
respectively) in directory root/d1/d2/.../dm. Note that n >= 1 and m >= 0. If m = 0, it means the 
...
part of the files with equal sizes (using MD5 for example). Only if the md5 is the same, we will compare the files byte by byte
