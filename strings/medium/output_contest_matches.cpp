...
commas(',') to represent the contest team pairing - parentheses('(' , ')') for pairing and commas(',') 
for partition. During the pairing process in each round, you always need to follow the strategy of making 
...
public:
    string findContestMatch(int n) {
        ...
        while (n > 1) {
            for (int i = 0; i < n / 2; i++) {
                //In the first iteration, we put everything.
                m[i] = "(" + m[i] + "," + m[n - 1 - i] + ")";
            }
            n /= 2;
        }
